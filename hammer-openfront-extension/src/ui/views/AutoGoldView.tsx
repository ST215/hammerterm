  ...
  return (
    <div className="flex items-center gap-1 min-w-20">
      <div className="flex-1 bg-hammer-bg rounded h-1 overflow-hidden">
        <div
          className="h-full bg-hammer-gold rounded"
          style={{ width: `${pct}%`, transition: "width 0.25s linear" }}
        />
      </div>
      <span className={`text-2xs font-mono shrink-0 ${remaining > 0 ? "text-hammer-gold" : "text-hammer-green"}`}>
...
export default function AutoGoldView() {
  ...
  // Gain rate tracking
  const prevRef = useRef({ value: myGold, ts: Date.now() });
  const [gainRate, setGainRate] = useState(0);
  useEffect(() => {
    const prev = prevRef.current;
    const dt = (Date.now() - prev.ts) / 1000;
    if (dt > 0.8) {
      setGainRate(Math.round((myGold - prev.value) / dt));
      prevRef.current = { value: myGold, ts: Date.now() };
    }
  }, [myGold]);
    ...
    <div className="font-mono text-hammer-text text-sm">
      ...
      <div className="bg-hammer-surface rounded p-2 border border-hammer-border">
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <div className="flex items-center gap-1">
            <span className="text-hammer-muted text-2xs">Min Keep</span>
            <span className="text-hammer-gold font-bold">{short(threshold)}</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-hammer-muted text-2xs">Gold</span>
            <span className="text-hammer-gold font-bold">{short(myGold)}</span>
          </div>
          <div className="flex items-center gap-1">
      ...
      <Section title="Live Preview">
        <div className="bg-hammer-surface rounded p-2 border border-hammer-border">
          <div className="grid grid-cols-2 gap-1 text-xs">
            ...
            <div className="flex justify-between">
              <span className="text-hammer-muted">vs Threshold</span>
              <span className={`font-bold ${belowThreshold ? "text-hammer-red" : "text-hammer-dim"}`}>
                {threshold > 0 ? `${Math.round((remaining / threshold) * 100)}%` : "no min"}
              </span>
            </div>
          </div>
              ...
              {resolvedTargets.map((rt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-hammer-gold text-xs font-bold shrink-0 mr-1">{short(perTarget)}g</span>
                    {ns ? (
