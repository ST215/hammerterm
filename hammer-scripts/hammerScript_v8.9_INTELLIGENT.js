...
// NEW: Intelligent send calculators with game mechanic recommendations
// NEW: Live countdown timers and next-send predictions
// NEW: +/- buttons replace sliders for better precision control
// FIXED: Timer conflicts - auto-troops and auto-gold work simultaneously
// FIXED: Number formatting matches game exactly (1.5M displays correctly)
// =====================================================================
(() => {
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.9</b> <span style="opacity:.85">INTELLIGENT</span></div>
  ...
  function autoDonateTroopsView() {
    ...
    if (!S.asTroopsAllTeamMode) {
      ...
      if (allTargets.length > 0) {
        html += '<div style="max-height:200px;overflow-y:auto;border:1px solid #2a3a55;border-radius:6px;padding:4px;background:#0d1520">'
        for (const p of allTargets) {
          const name = p.displayName || p.name || 'Unknown'
          const isSelected = S.asTroopsTargets.includes(name)
          html += `<button class="${isSelected ? 'active' : ''}" data-toggle-troop-target="${esc(name)}" style="display:flex;justify-content:space-between;width:100%;text-align:left;margin:2px 0">`
    ...
    if (!S.asGoldAllTeamMode) {
      ...
      if (allTargets.length > 0) {
        ...
        for (const p of allTargets) {
          ...
          html += `<button class="${isSelected ? 'active' : ''}" data-toggle-gold-target="${esc(name)}" style="display:flex;justify-content:space-between;width:100%;text-align:left;margin:2px 0">`
          html += `<div>${esc(name)}</div>`
