// =====================================================================
// HAMMER v8.7 "RELOADABLE EDITION"
// NEW: Full support for mid-game reruns - properly handles existing Workers/WebSockets
// Fixed: Worker/WebSocket discovery, event listener cleanup, canvas detection
// Fixed: Auto-troops/gold log display without scroll views
// =====================================================================
(() => {
  ...
  // NEW: Setup Worker wrapper for future instances
  ...
  // NEW: Discover and wrap EXISTING Worker instances
  ...
  // NEW: Discover and wrap EXISTING WebSocket instances
  ...
  try {
    ...
    // NEW: Immediate canvas detection for mid-game reruns
    setTimeout(() => {
      if (!targetCanvas) {
  ...
  eventCleanup.push(() => window.removeEventListener('keydown', keydownHandler))
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.7</b> <span style="opacity:.85">Reloadable</span></div>
  ...
  ui.querySelector('#hm-export').onclick = () => {
    ...
    a.download = `hammer_v8.6_${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  function autoDonateTroopsView() {
    ...
    if (!S.asTroopsAllTeamMode) {
      ...
      if (S.asTroopsTargets.length > 0) {
        ...
        html += '<div class="muted">No targets selected. Use ALT+M on map</div>'
      }
      html += '</div>'
    } else {
      const teammates = getTeammates()
    ...
    // Activity Log - FIXED: No scroll views, just show recent entries
    if (S.asTroopsLog.length > 0) {
      html += '<div class="box">'
      html += '<div class="title" style="margin-top:0">📋 Recent Activity (Last 10)</div>'
  ...
  window.__HAMMER__ = { cleanup, ui: { root: ui }, version: '8.7' }
  ...
  console.log('%c[HAMMER]%c v8.7 RELOADABLE ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
