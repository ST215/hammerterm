// =====================================================================
// HAMMER v7.1 "CLARITY EDITION"
// Better UX, Tag targeting, Enhanced analytics, Strategic ports
// =====================================================================
(() => {
  // Hard reset
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v7.1</b> <span style="opacity:.85">Clarity Edition</span></div>
    ...
    <div id="hm-body" style="height:${SIZES[S.sizeIdx].bodyH}px;overflow:auto;padding:10px">
      <style>
        ...
        #hammer-v7 .example{background:#0d1520;border-left:3px solid #4a6894;padding:6px 8px;margin:6px 0;font-size:11px;line-height:1.5}
        #hammer-v7 .example-title{color:#7bb8ff;font-weight:700;margin-bottom:4px}
        ...
        #hammer-v7 .stat-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin:8px 0}
        #hammer-v7 .stat-card{background:#0d1520;border:1px solid #2a3a55;border-radius:6px;padding:10px}
        #hammer-v7 .stat-label{color:#9bb0c8;font-size:10px;text-transform:uppercase;margin-bottom:4px}
        #hammer-v7 .stat-value{color:#7ff2a3;font-size:18px;font-weight:700}
        #hammer-v7 .recommendation{background:#1a2a1f;border-left:3px solid #4a8864;padding:8px;margin:6px 0;font-size:11px}
        #hammer-v7 .warning{background:#2a1f1a;border-left:3px solid #ff8b94;padding:8px;margin:6px 0;font-size:11px}
      </style>
  ...
  ui.querySelector('#hm-export').onclick = () => {
    ...
    a.download = `hammer_v7.1_${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  function portsRows() {
    const arr = [...S.ports.entries()]
      ...
      .sort((a, b) => b.gpm - a.gpm)  // Sort by GPM instead of total gold
      .slice(0, 40)

    if (!arr.length) return '<span class="muted">No port trades yet. Trade with ports to see analysis.</span>'

    // Calculate insights
    const totalGPM = arr.reduce((sum, r) => sum + r.gpm, 0)
    const avgGPM = arr.length ? Math.round(totalGPM / arr.length) : 0
    const bestPort = arr[0]
    const badPorts = arr.filter(r => r.avg > 120 && r.gpm < 2000)

    let html = `
      <div class="stat-grid">
        <div class="stat-card">
          <div class="stat-label">Total GPM</div>
          <div class="stat-value">${short(totalGPM)}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Average GPM</div>
          <div class="stat-value">${short(avgGPM)}</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          <div class="stat-label">Best Port</div>
          <div class="stat-value mono" style="font-size:12px">${bestPort ? short(bestPort.gpm) : '-'}</div>
        </div>
      </div>
    `

    if (bestPort) {
      html += `<div class="recommendation">💡 <b>Best Performer:</b> ${esc(bestPort.name)} generates ${short(bestPort.gpm)} GPM</div>`
    }

    if (badPorts.length) {
      html += `<div class="warning">⚠️ <b>Low Efficiency:</b> ${badPorts.length} port${badPorts.length > 1 ? 's' : ''} with high travel time and low GPM. Consider embargoing.</div>`
    }

    html += '<div class="title">Port Performance</div>'
    html += arr.map(r => {
      const quality = r.gpm >= avgGPM ? 'color:#7ff2a3' : r.gpm < 2000 && r.avg > 120 ? 'color:#ff8b94' : ''
      return `<div class="row mono"><span>${esc(r.name)}</span><span style="${quality}"><b>${short(r.gpm)} gpm</b> • ${r.avg}s avg • ${short(r.totalGold)}💰 total</span></div>`
    }).join('')
  ...
  function goldRateView() {
    ...
    const tiles = me ? me.tilesOwned || 0 : 0
    const cities = me ? cityLevelSumByOwner.get(me.smallID) || 0 : 0
    const gpm60 = S.gpm60 || 0
    const gpm120 = S.gpm120 || 0

    // Calculate projections
    const goldIn5Min = Math.round(gold + (gpm60 * 5))
    const goldIn10Min = Math.round(gold + (gpm60 * 10))

    return `
      <div class="title">Current Resources</div>
      <div class="stat-grid">
        <div class="stat-card">
          <div class="stat-label">Gold</div>
          <div class="stat-value">${short(gold)}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Troops</div>
          <div class="stat-value">${short(troops / 10)}${maxT ? ` <span class="muted" style="font-size:12px">/ ${short(maxT / 10)}</span>` : ''}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Tiles</div>
          <div class="stat-value">${tiles}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">City Levels</div>
          <div class="stat-value">${cities}</div>
        </div>
      </div>

      <div class="title">Gold Production Rates</div>
      <div class="stat-grid">
        <div class="stat-card">
          <div class="stat-label">Gold/sec (30s)</div>
          <div class="stat-value">${(S.gps30 || 0).toFixed(2)}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Gold/min (60s)</div>
          <div class="stat-value">${short(gpm60)}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Gold/min (120s)</div>
          <div class="stat-value">${short(gpm120)}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Gold/hour</div>
          <div class="stat-value">${short(gpm60 * 60)}</div>
        </div>
      </div>

      <div class="title">Projections</div>
      <div class="box">
        <div class="row"><span>Gold in 5 min:</span><span class="mono" style="color:#7ff2a3"><b>${short(goldIn5Min)}</b></span></div>
        <div class="row"><span>Gold in 10 min:</span><span class="mono" style="color:#7ff2a3"><b>${short(goldIn10Min)}</b></span></div>
      </div>

      ${maxT && troops ? `
      <div class="title">Troop Capacity</div>
      <div class="box">
        <div class="row"><span>Current / Max:</span><span class="mono">${short(troops / 10)} / ${short(maxT / 10)}</span></div>
        <div class="row"><span>Capacity Used:</span><span class="mono" style="color:${(troops / maxT * 100) > 80 ? '#ff8b94' : '#7ff2a3'}"><b>${((troops / maxT) * 100).toFixed(1)}%</b></span></div>
      </div>
      ` : ''}

      <div class="title">Data Quality</div>
      <div class="muted">Tracking ${goldHistory.length} samples over 2 minutes</div>
      ${goldHistory.length < 10 ? '<div class="warning">⚠️ Collecting data... More accurate after 30 seconds</div>' : ''}
    `
  }
  ...
  function autoDonateTroopsView() {
    ...
    return `
      ...
      <div class="example">
        ...
        <b>Ratio:</b> What % of your current troops to send<br>
        <b>Threshold:</b> Only donate when troops ≥ this % of max capacity<br><br>
        <b>Example:</b> Ratio 20%, Threshold 50%<br>
        → When you have 50k/100k troops (50%), send 10k (20% of 50k)
      </div>
      ...
      <div class="box">
        ...
        <div class="row"><span>Current Troops:</span><span class="mono">${short(troops / 10)} <span class="muted">(${pct}% of max)</span></span></div>
        <div class="row"><span>Max Capacity:</span><span class="mono">${short(maxT / 10)}</span></div>
        <div class="row">
          <span>Target:</span>
      ...
      <div class="box">
        <div class="row">
          ...
          <span class="muted">= ${short(troops * (S.asTroopsRatio / 100) / 10)} troops</span>
        </div>
        <div class="row">
          <label>Threshold (%):</label>
          <input id="as-troops-threshold" type="number" min="1" max="100" value="${S.asTroopsThreshold}" style="width:80px">
          <span class="muted">= ${short(maxT * (S.asTroopsThreshold / 100) / 10)} troops</span>
        </div>
        <div class="row">
      ...
      <div class="title">Quick Targets</div>
      <div class="box">
        <button id="as-troops-allplayers" style="margin:2px">🌍 All Allies</button>
        ${S.myTag ? `<button id="as-troops-tagmates" style="margin:2px">🏷️ All [${S.myTag}] Players</button>` : ''}
        <div class="muted" style="margin-top:6px">Or use ALT+M to hover-capture, or select below:</div>
      </div>
      ...
      ${tagmates.length > 0 ? `
        <div class="title">Clan [${S.myTag}] (${tagmates.length})</div>
        <div class="player-list">
          ${tagmates.map(p => `<button class="player-btn" data-name="${esc(p.displayName || p.name)}">${esc(p.displayName || p.name)} <span class="muted">(ID: ${p.smallID})</span></button>`).join('')}
        </div>
      ` : ''}
  ...
  function autoDonateGoldView() {
    ...
    return `
      ...
      <div class="example">
        <div class="example-title">📖 How It Works</div>
        <b>Amount:</b> How much gold to send per donation<br>
        <b>Threshold:</b> Only donate when your gold ≥ this amount<br><br>
        <b>Example:</b> Amount 10k, Threshold 100k<br>
        → When you have ≥100k gold, send 10k
      </div>
      ...
      <div class="title">Quick Targets</div>
      <div class="box">
        <button id="as-gold-allplayers" style="margin:2px">🌍 All Allies</button>
        ${S.myTag ? `<button id="as-gold-tagmates" style="margin:2px">🏷️ All [${S.myTag}] Players</button>` : ''}
        <div class="muted" style="margin-top:6px">Or use ALT+M to hover-capture, or select below:</div>
      </div>
      ...
      ${tagmates.length > 0 ? `
        <div class="title">Clan [${S.myTag}] (${tagmates.length})</div>
        <div class="player-list">
          ${tagmates.map(p => `<button class="player-btn-gold" data-name="${esc(p.displayName || p.name)}">${esc(p.displayName || p.name)} <span class="muted">(ID: ${p.smallID})</span></button>`).join('')}
        </div>
      ` : ''}
  ...
  function embargoView() {
    ...
    return `
      ...
      <div class="help">Note: Embargoes may take a few seconds to process due to game server rate limiting.</div>
  ...
  function render() {
    ...
    else if (S.view === 'ports') html = `<div class="title">Ports — Strategic Analysis</div>${portsRows()}`
    ...
    if (S.view === 'autotroops') {
      ...
      const tagBtn = ui.querySelector('#as-troops-tagmates')

      if (ratio) ratio.oninput = () => render()  // Live preview
      if (thresh) thresh.oninput = () => render()  // Live preview
      ...
      if (tagBtn) tagBtn.onclick = () => { S.asTroopsTarget = 'AllTagMates'; showStatus(`✅ Target: All [${S.myTag}]`); render() }
    ...
    if (S.view === 'autogold') {
      ...
      const tagBtn = ui.querySelector('#as-gold-tagmates')
      ...
      if (tagBtn) tagBtn.onclick = () => { S.asGoldTarget = 'AllTagMates'; showStatus(`✅ Target: All [${S.myTag}]`); render() }
  ...
  console.log('%c[HAMMER]%c v7.1 Clarity Edition ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
