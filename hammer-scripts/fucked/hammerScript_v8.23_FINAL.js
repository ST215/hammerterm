...
// FIXED: All data sources aligned with v8.21
// FIXED: Feed/port/transfer tracking restored
// FIXED: AutoTroops actually sends troops
// FIXED: AutoGold initializes and activates correctly
// FIXED: Number formatting verified (K/M suffixes correct)
// NEW: Weak Players tab for easy target identification
// NEW: Large team pagination (15 players per page) from v8.22
// Previous fixes from v8.21:
// - Auto-gold performance (now as snappy as auto-troops)
// - Auto-gold target selection now working properly
// - Number formatting bugs (150k no longer shows as 1.5M)
// - Multi-feed calculations (no more infinity bugs)
// - Nested scroll bounce issues removed
// - Excessive refresh rates in Command View and Threat displays
// =====================================================================
(() => {
  ...
  function computePlayerMetrics(force = false) {
    ...
    for (const player of playersById.values()) {
      ...
      const troopFillPct = troopCap > 0 ? Math.min(100, Math.round((safeNum(player.troops) / troopCap) * 100)) : 0
      const portIncome = portRec ? portRec.received : 0
      const portSpent = portRec ? portRec.sent : 0
      const powerScore = safeNum(player.troops) +
  ...
  function worldToScreen(worldX, worldY) {
    ...
    const pt = matrix.transformPoint(createPoint(worldX * sx, worldY * sy))
  ...
  function drawOverlays() {
    ...
    if (S.samOverlayEnabled) {
      ...
      for (const sam of samUnits.values()) {
        ...
        if (!coords) continue
        if (!worldTilesWidth || !worldTilesHeight) continue

        const center = worldToScreen(coords.x + 0.5, coords.y + 0.5)
        const edge = worldToScreen(coords.x + 0.5 + SAM_RANGE_TILES, coords.y + 0.5)
        const radiusPixels = Math.hypot(edge.x - center.x, edge.y - center.y)
        if (!Number.isFinite(radiusPixels) || radiusPixels <= 0) continue
    ...
    if (S.atomOverlayEnabled) {
      ...
      const center = worldToScreen(worldPoint.x, worldPoint.y)
      const innerEdge = worldToScreen(worldPoint.x + ATOM_INNER, worldPoint.y)
      const outerEdge = worldToScreen(worldPoint.x + ATOM_OUTER, worldPoint.y)
      const innerRadius = Math.hypot(innerEdge.x - center.x, innerEdge.y - center.y)
      const outerRadius = Math.hypot(outerEdge.x - center.x, outerEdge.y - center.y)
    ...
    if (S.hydrogenOverlayEnabled) {
      const worldPoint = screenToWorld(lastMouseClient.x, lastMouseClient.y)
      const center = worldToScreen(worldPoint.x, worldPoint.y)
      const innerEdge = worldToScreen(worldPoint.x + HYDROGEN_INNER, worldPoint.y)
      const outerEdge = worldToScreen(worldPoint.x + HYDROGEN_OUTER, worldPoint.y)
      const innerRadius = Math.hypot(innerEdge.x - center.x, innerEdge.y - center.y)
      const outerRadius = Math.hypot(outerEdge.x - center.x, outerEdge.y - center.y)
  ...
  const tabs = ['command', 'summary', 'stats', 'aiinsights', 'threat', 'power', 'weak', 'ports', 'feed', 'goldrate', 'alliances', 'autotroops', 'autogold', 'overlays', 'embargo', 'hotkeys']
  const tabLabels = {
    command: 'Command',
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.23</b> <span style="opacity:.85">FINAL</span></div>
  ...
  function commandCenterView() {
    ...
    const weakPlayers = lastPlayers
      .filter(p => p.id !== mySmallID && !asIsAlly(p.id))
      .map(p => {
        const troops = p.troops || 0
        const tiles = p.tilesOwned || 0
        const production = tiles * 250 // estimate base production
        const weakness = (1000000 - troops) + (500 - tiles) * 1000 // higher = weaker
        return { name: p.displayName || p.name, troops, tiles, production, weakness, id: p.id }
      })
  ...
  function weakPlayersView() {
    let html = '<div class="title">🎯 Weak Targets</div>'
    html += '<div class="help">Easy targets for expansion (low troops, low production)</div>'
    ...
    const weakPlayers = lastPlayers
      .filter(p => p.id !== me.id && p.smallID !== mySmallID && !asIsAlly(p.id) && p.isAlive)
      .map(p => {
        const troops = p.troops || 0
        const gold = p.gold || 0
        const tiles = p.tilesOwned || 0
        const production = tiles * 250 // estimate base production
        const troopCap = estimateMaxTroops(tiles, p.smallID)
        const troopPct = troopCap > 0 ? Math.round((troops / troopCap) * 100) : 0
        // Higher weakness score = easier target
        const weakness = (1000000 - troops) + (500 - tiles) * 1000 + (100000 - gold) * 0.5
        return {
    ...
    if (!weakPlayers.length) {
      return html + '<div class="muted">No weak targets identified yet.</div>'
    }

    html += '<div class="box">'
    for (const target of weakPlayers) {
      html += `<div class="row" style="background:#0d1520;padding:8px;border-radius:4px;margin:6px 0">`
      html += `<div style="flex:1">`
      html += `<div style="font-weight:700">${esc(target.name)}</div>`
      html += `<div class="muted mono" style="font-size:10px">${target.tiles} tiles · ${target.troopPct}% troops</div>`
      html += `</div>`
      html += `<div style="text-align:right">`
      html += `<div class="mono" style="color:#7bb8ff">${short(target.troops)} 🪖</div>`
      html += `<div class="mono muted" style="font-size:10px">${short(target.gold)} 💰</div>`
      html += `</div>`
      html += '</div>'
    }
    html += '</div>'
  ...
  function aiInsightsView() {
    ...
    if (meMetrics) {
      ...
      const lowTeammate = teammates.find(t => {
        ...
        return cap > 0 && (t.troops / cap) * 100 < 35
      })
      if (lowTeammate) {
  ...
  function goldRateView() {
    ...
    if (meMetrics) {
      const advice = buildAdviceForPlayer(meMetrics)
      html += '<div class="box"><div class="title" style="margin-top:0">🛠 Build Advice</div>'
      if (!advice.length) {
        html += '<div class="muted">No specific recommendations — keep expanding steadily.</div>'
      } else {
        advice.slice(0, 4).forEach(line => {
          html += `<div class="recommendation" title="Generated from your current income trend.">💡 ${esc(line)}</div>`
        })
      }
      html += '</div>'
    }

    html += '<div class="help" style="margin-top:12px">💡 Higher rates point to active ports or fresh territory gains.</div>'
  ...
  function autoDonateTroopsView() {
    ...
    if (!S.asTroopsAllTeamMode) {
      ...
      if (allTargets.length > 0) {
        const pageSize = S.targetPageSize || 15
        const totalPages = Math.ceil(allTargets.length / pageSize)
        const currentPage = Math.max(0, Math.min(S.asTroopsPage || 0, totalPages - 1))
        const startIdx = currentPage * pageSize
        const endIdx = Math.min(startIdx + pageSize, allTargets.length)
        const pageTargets = allTargets.slice(startIdx, endIdx)

        html += '<div class="help" style="margin-top:8px">Available Targets (click to toggle):</div>'
        html += `<div class="help">Showing ${startIdx + 1}-${endIdx} of ${allTargets.length} players</div>`

        // Pagination controls
        if (totalPages > 1) {
          html += '<div style="display:flex;gap:4px;margin:8px 0">'
          html += `<button id="at-page-prev" ${currentPage === 0 ? 'disabled' : ''}>◀ Prev</button>`
          html += `<div style="flex:1;text-align:center;padding:4px" class="mono">Page ${currentPage + 1} / ${totalPages}</div>`
          html += `<button id="at-page-next" ${currentPage === totalPages - 1 ? 'disabled' : ''}>Next ▶</button>`
          html += '</div>'
        }

        html += '<div id="at-target-list">'
        for (const p of pageTargets) {
          ...
          html += `<div class="mono" style="color:#7bb8ff" data-troop-num="${p.id}">${short(p.troops)}</div>`
          html += '</div>'
          html += '</div>'
        }
        html += '</div>'
  ...
  function autoDonateGoldView() {
    ...
    if (!S.asGoldAllTeamMode) {
      ...
      if (allTargets.length > 0) {
        const pageSize = S.targetPageSize || 15
        const totalPages = Math.ceil(allTargets.length / pageSize)
        const currentPage = Math.max(0, Math.min(S.asGoldPage || 0, totalPages - 1))
        const startIdx = currentPage * pageSize
        const endIdx = Math.min(startIdx + pageSize, allTargets.length)
        const pageTargets = allTargets.slice(startIdx, endIdx)
        ...
        if (totalPages > 1) {
          html += '<div style="display:flex;gap:4px;margin:8px 0">'
          html += `<button id="ag-page-prev" ${currentPage === 0 ? 'disabled' : ''}>◀ Prev</button>`
          html += `<div style="flex:1;text-align:center;padding:4px" class="mono">Page ${currentPage + 1} / ${totalPages}</div>`
          html += `<button id="ag-page-next" ${currentPage === totalPages - 1 ? 'disabled' : ''}>Next ▶</button>`
          html += '</div>'
        }

        html += '<div id="ag-target-list">'
        for (const p of pageTargets) {
          const name = p.displayName || p.name || 'Unknown'
  ...
  function render() {
    ...
    // Auto Troops pagination
    const atPagePrev = ui.querySelector('#at-page-prev')
    const atPageNext = ui.querySelector('#at-page-next')
    if (atPagePrev) {
      atPagePrev.onclick = () => {
        S.asTroopsPage = Math.max(0, (S.asTroopsPage || 0) - 1)
        render()
      }
    }
    if (atPageNext) {
      atPageNext.onclick = () => {
        S.asTroopsPage = (S.asTroopsPage || 0) + 1
        render()
      }
    }

    // Auto Gold pagination
    const agPagePrev = ui.querySelector('#ag-page-prev')
    const agPageNext = ui.querySelector('#ag-page-next')
    if (agPagePrev) {
      agPagePrev.onclick = () => {
        S.asGoldPage = Math.max(0, (S.asGoldPage || 0) - 1)
        render()
      }
    }
    if (agPageNext) {
      agPageNext.onclick = () => {
        S.asGoldPage = (S.asGoldPage || 0) + 1
        render()
      }
    }
    ...
    // ===== SMOOTH SCROLL FIX: Update only numbers, not entire DOM =====
    // This prevents scroll position jumping by keeping DOM structure stable
    if (S.view === 'autotroops') {
      ui.querySelectorAll('[data-troop-num]').forEach(span => {
        const playerId = num(span.getAttribute('data-troop-num'))
        const player = playersById.get(playerId)
        if (player) {
          span.textContent = short(player.troops)
        }
      })
    } else if (S.view === 'autogold') {
      ui.querySelectorAll('[data-gold-num]').forEach(span => {
        const playerId = num(span.getAttribute('data-gold-num'))
        const player = playersById.get(playerId)
        if (player) {
          span.textContent = short(player.gold || 0) + ' 💰'
        }
      })
    }
  }
  ...
  window.__HAMMER__ = { cleanup, ui: { root: ui }, version: '8.23' }
  ...
  console.log('%c[HAMMER]%c v8.23 FINAL ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
  console.log('[HAMMER] Status:', initMessages.join(' | '))
  console.log('[HAMMER] ✅ All data sources aligned with v8.21!')
  console.log('[HAMMER] ✅ Feed/port/transfer tracking restored!')
  console.log('[HAMMER] ✅ Weak player targeting added!')
  console.log('[HAMMER] ✅ Large team pagination (15 players per page)!')
