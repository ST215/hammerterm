// =====================================================================
// HAMMER v8.2 "ULTIMATE EDITION"
// Multi-target, visual sliders, teammate overlay, port insights, all fixes
// =====================================================================
(() => {
  // Hard reset
  ...
  const TEAMMATE_CIRCLE_TILES = 50  // Circle radius for teammate overlay
  ...
  const S = {
    ...
    teammateOverlayEnabled: true,  // ON by default

    // Auto-donate troops state - NOW WITH MULTI-TARGET
    asTroopsRunning: false,
    asTroopsTargets: [],  // Array of player names
    asTroopsRatio: 20,
    asTroopsThreshold: 50,
    asTroopsLastSend: {},
    asTroopsCooldownSec: 10,  // Now in seconds!
    asTroopsLog: [],
    asTroopsAllTeamMode: false,  // New: send to entire team

    // Auto-donate gold state - NOW WITH MULTI-TARGET
    asGoldRunning: false,
    asGoldTargets: [],  // Array of player names
    asGoldAmount: 10000,
    asGoldThreshold: 100000,
    asGoldLastSend: {},
    asGoldCooldownSec: 10,  // Now in seconds!
    asGoldLog: [],
    asGoldAllTeamMode: false  // New: send to entire team
  }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.altKey && e.code === 'KeyT') {
      e.preventDefault()
      S.teammateOverlayEnabled = !S.teammateOverlayEnabled
      showStatus(S.teammateOverlayEnabled ? '✅ Teammate Overlay ON' : '⏹️ Teammate Overlay OFF')
      shouldRender = true
      return
    }
  ...
  function asResolveTargets() {
    if (S.asTroopsAllTeamMode) {
      // AllTeam mode: send to all teammates
      return getTeammates().map(p => ({ id: p.id, name: p.displayName || p.name }))
    }

    // Multi-target mode: resolve each target name
    const resolved = []
  ...
  function asTroopsTick() {
    ...
    // Send to each target that's off cooldown
  ...
  function drawOverlays() {
    ...
    // Teammate overlay (ON by default)
    if (S.teammateOverlayEnabled) {
      ctx.save()
      const me = readMyPlayer()
      if (me) {
        let teammates = getTeammates()
        if (S.myTag) {
          teammates = [...teammates, ...getTagMates()]
          // Deduplicate
          teammates = teammates.filter((p, i, arr) => arr.findIndex(x => x.id === p.id) === i)
        }

        for (const tm of teammates) {
          // Find a tile owned by this teammate
          let tmTileRef = null
          for (const [ref, ownerSmall] of tileOwnerByRef) {
            if (ownerSmall === tm.smallID) {
              tmTileRef = ref
              break
            }
          }
          ...
          if (tmTileRef !== null) {
            const coords = tileCoordsToWorld(tmTileRef)
            if (coords) {
              const pos = worldToScreen(coords.x, coords.y)
              const radiusPixels = TEAMMATE_CIRCLE_TILES * (screenCanvasWidth / worldTilesWidth) * currentTransform.a

              ctx.strokeStyle = 'rgba(127, 242, 163, 0.8)'  // Green for teammates
              ctx.lineWidth = 3
              ctx.beginPath()
              ...
              // Draw teammate name
              ctx.fillStyle = 'rgba(127, 242, 163, 0.9)'
              ctx.font = `${14 * currentTransform.a}px Consolas`
              ctx.fillText(tm.displayName || tm.name || 'Teammate', pos.x, pos.y - radiusPixels - 5)
            }
          }
        }
      }
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.2</b> <span style="opacity:.85">Ultimate</span></div>
  ...
  ui.querySelector('#hm-export').onclick = () => {
    ...
    a.download = `hammer_v8.2_${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  function overlaysView() {
    ...
    html += '<div class="box">'
    html += `<div class="row"><div>Teammate Overlay</div><button id="ov-teammate">${S.teammateOverlayEnabled ? 'ON' : 'OFF'}</button><span class="hotkey">ALT+T</span></div>`
    html += '<div class="help">Green circles around teammates/clan mates (ON by default)</div>'
    html += '</div>'
  ...
  function hotkeysView() {
    ...
    html += '<div class="row"><div>Teammate Overlay</div><span class="hotkey">ALT+T</span></div>'
  ...
  function render() {
    ...
    // Overlay toggles
    const ovTeammate = ui.querySelector('#ov-teammate')
    const ovSam = ui.querySelector('#ov-sam')
    const ovAtom = ui.querySelector('#ov-atom')
    const ovHydrogen = ui.querySelector('#ov-hydrogen')

    if (ovTeammate) ovTeammate.onclick = () => { S.teammateOverlayEnabled = !S.teammateOverlayEnabled; shouldRender = true }
  ...
  window.__HAMMER__ = { cleanup, ui: { root: ui }, version: '8.2' }

  render()
  console.log('%c[HAMMER]%c v8.2 ULTIMATE ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
  console.log('[HAMMER] ✅ Multi-target selection, visual sliders, teammate overlay!')
  console.log('[HAMMER] ✅ Port insights, AllTeam mode, cooldown in seconds!')
  console.log('[HAMMER] Press ALT+M to add targets, ALT+F to toggle, ALT+T for teammate overlay')
})()
