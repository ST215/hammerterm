// =====================================================================
// HAMMER v8.0 "ULTIMATE EDITION"
// Fixed scroll bouncing, input bugs, Chrome Extension ready, AI insights
// =====================================================================
(() => {
  // Hard reset
  ...
  function bump(map, key) {
    if (!map.has(key)) map.set(key, { gold: 0, troops: 0, count: 0, last: null, isPort: false })
    return map.get(key)
  }
  ...
  function processDisplayMessage(msg) {
    ...
    if (mt === MessageType.RECEIVED_TROOPS_FROM_PLAYER) {
      ...
      if (m) {
        ...
        if (from && amt > 0) {
          const r = bump(S.inbound, from.id)
          r.gold += amt; r.count++; r.last = nowDate(); r.isPort = true
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.0</b> <span style="opacity:.85">Ultimate Edition</span></div>
  ...
  ui.querySelector('#hm-export').onclick = () => {
    ...
    a.download = `hammer_v8.0_${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  function summaryView() {
    const me = readMyPlayer()
    const myName = me ? (me.displayName || me.name || 'You') : 'You'
    ...
    for (const k of inKeys) {
      ...
      if (r.isPort) totalInPort += r.gold
    }
  ...
  function aiInsightsView() {
    ...
    let html = '<div class="title">🤖 Claude AI Insights - Fun Analytics</div>'
    html += '<div class="help">Strategic insights and fun data analysis powered by Claude AI</div>'
  ...
  function autoDonateTroopsView() {
    ...
    html += '<div class="box">'
    html += `<div class="row"><div>Target</div><input id="at-target" value="${esc(S.asTroopsTarget)}" placeholder="Player name or AllPlayers" style="flex:1"></div>`
  ...
  function autoDonateGoldView() {
    ...
    html += '<div class="box">'
    html += `<div class="row"><div>Target</div><input id="ag-target" value="${esc(S.asGoldTarget)}" placeholder="Player name or AllPlayers" style="flex:1"></div>`
  ...
  function render() {
    ...
    // Wire up event handlers for auto-troops
    const atTarget = ui.querySelector('#at-target')
    ...
    if (atTarget) atTarget.oninput = () => S.asTroopsTarget = atTarget.value
    if (atRatio) atRatio.oninput = () => S.asTroopsRatio = num(atRatio.value)
    if (atThreshold) atThreshold.oninput = () => S.asTroopsThreshold = num(atThreshold.value)
    if (atCooldown) atCooldown.oninput = () => S.asTroopsCooldownMs = num(atCooldown.value)
    ...
    // Wire up event handlers for auto-gold
    const agTarget = ui.querySelector('#ag-target')
    ...
    if (agTarget) agTarget.oninput = () => S.asGoldTarget = agTarget.value
    if (agAmount) agAmount.oninput = () => S.asGoldAmount = num(agAmount.value)
    if (agThreshold) agThreshold.oninput = () => S.asGoldThreshold = num(agThreshold.value)
    if (agCooldown) agCooldown.oninput = () => S.asGoldCooldownMs = num(agCooldown.value)
  ...
  window.__HAMMER__ = { cleanup, ui: { root: ui }, version: '8.0' }

  render()
  console.log('%c[HAMMER]%c v8.0 Ultimate Edition ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
