// =====================================================================
// HAMMER v4.1 "ULTIMATE EDITION"
// Full Worker interception + All features + Enhanced UI + Overlays
// =====================================================================
(() => {
  // Hard reset
  ...
  const tabs = ['inbound', 'outbound', 'ports', 'feed', 'goldrate', 'alliances', 'autotroops', 'autogold', 'overlays', 'embargo']
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v4.1</b> <span style="opacity:.85">Ultimate</span></div>
    ...
    <div id="hm-body" style="height:${SIZES[S.sizeIdx].bodyH}px;overflow:auto;padding:10px">
      <style>
        ...
        #hammer-v4 .help{color:#7bb8ff;font-size:11px;line-height:1.4;margin:4px 0}
        ...
        #hammer-v4 input:focus{outline:2px solid #4a6894;background:#152030}
      </style>
  ...
  ui.querySelector('#hm-export').onclick = () => {
    ...
    a.download = `hammer_v4.1_${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  function autoDonateTroopsView() {
    ...
    return `
      <div class="title">Auto-Donate Troops</div>
      <div class="help">
        Automatically donates troops when your capacity exceeds threshold.<br>
        <b>Target:</b> Player name, smallID, or "AllPlayers" for all allies<br>
        <b>Ratio:</b> % of current troops to send (1-100%)<br>
        <b>Threshold:</b> Only donate when troops >= this % of max capacity
      </div>
      <div class="box">
        <div class="row">
          <span>Status:</span>
      ...
      <div class="box">
        <div class="row">
          <label>Target:</label>
          <input id="as-troops-target" type="text" value="${esc(S.asTroopsTarget)}" placeholder="PlayerName, smallID, or AllPlayers" style="flex:1">
        </div>
        <div class="row">
          <label>Ratio (%):</label>
  ...
  function autoDonateGoldView() {
    ...
    return `
      ...
      <div class="help">
        Automatically donates fixed gold amounts when you have enough.<br>
        <b>Target:</b> Player name, smallID, or "AllPlayers" for all allies<br>
        <b>Amount:</b> Fixed gold amount to send each time<br>
        <b>Threshold:</b> Only donate when your gold >= this amount
      </div>
      <div class="box">
        <div class="row">
          <span>Status:</span>
      ...
      <div class="box">
        <div class="row">
          <label>Target:</label>
          <input id="as-gold-target" type="text" value="${esc(S.asGoldTarget)}" placeholder="PlayerName, smallID, or AllPlayers" style="flex:1">
        </div>
        <div class="row">
          <label>Amount:</label>
  ...
  function overlaysView() {
    return `
      ...
      <div class="help">
        Toggle visual overlays on the game map
      </div>
      <div class="box">
        <div class="row">
          <label>SAM Launcher Range (70 tiles):</label>
          <button id="toggle-sam" style="background:${S.samOverlayEnabled ? '#2a5244' : '#0e1a2f'}">${S.samOverlayEnabled ? 'ON' : 'OFF'}</button>
        </div>
        <div class="muted">Shows red circles around all SAM launchers</div>
      </div>
      <div class="box">
        <div class="row">
          <label>Atom Bomb Radius:</label>
          <button id="toggle-atom" style="background:${S.atomOverlayEnabled ? '#2a5244' : '#0e1a2f'}">${S.atomOverlayEnabled ? 'ON' : 'OFF'}</button>
        </div>
        <div class="muted">Inner: 12 tiles (kill), Outer: 30 tiles (damage). Follows mouse cursor.</div>
      </div>
      <div class="box">
        <div class="row">
          <label>Hydrogen Bomb Radius:</label>
          <button id="toggle-hydrogen" style="background:${S.hydrogenOverlayEnabled ? '#2a5244' : '#0e1a2f'}">${S.hydrogenOverlayEnabled ? 'ON' : 'OFF'}</button>
        </div>
        <div class="muted">Inner: 80 tiles (kill), Outer: 100 tiles (damage). Follows mouse cursor.</div>
      </div>
    `
  }
  ...
  function render() {
    ...
    if (S.view === 'autotroops') {
      const tgt = ui.querySelector('#as-troops-target')
      ...
      if (startBtn) startBtn.onclick = () => {
        if (tgt) S.asTroopsTarget = tgt.value.trim()
        ...
        if (!S.asTroopsTarget) return alert('Enter a target!')
        asTroopsStart()
        render()
      }
      if (stopBtn) stopBtn.onclick = () => { asTroopsStop(); render() }
    }

    // Auto-donate gold event listeners
    if (S.view === 'autogold') {
      const tgt = ui.querySelector('#as-gold-target')
      ...
      if (startBtn) startBtn.onclick = () => {
        if (tgt) S.asGoldTarget = tgt.value.trim()
        if (amt) S.asGoldAmount = Math.max(1000, num(amt.value))
        if (thresh) S.asGoldThreshold = Math.max(1000, num(thresh.value))
        if (!S.asGoldTarget) return alert('Enter a target!')
        asGoldStart()
        render()
      }
      if (stopBtn) stopBtn.onclick = () => { asGoldStop(); render() }
    }
    ...
    if (S.view === 'overlays') {
      ...
      if (samBtn) samBtn.onclick = () => {
        S.samOverlayEnabled = !S.samOverlayEnabled
        render()
      }
      if (atomBtn) atomBtn.onclick = () => {
        S.atomOverlayEnabled = !S.atomOverlayEnabled
        render()
      }
      if (hydrogenBtn) hydrogenBtn.onclick = () => {
  ...
  console.log('%c[HAMMER]%c v4.1 Ultimate Edition ready! 🔨', 'color:#deb887;font-weight:bold', 'color:inherit')
  console.log('[HAMMER] Features: Donations, Gold Rate, Auto-Donate (Troops & Gold), Overlays (SAM, Atom, Hydrogen), Alliances, Embargoes')
})()
