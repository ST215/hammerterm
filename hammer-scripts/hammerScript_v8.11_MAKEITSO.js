...
// FIXED: Number display bug (33.6K now shows correctly, not 336k)
// FIXED: Gold sender event handlers now work properly
// FIXED: True smooth scrolling - stable DOM, only numbers update
// INCLUDES: All v8.9 + v8.10 features
// =====================================================================
(() => {
  ...
  const short = v => {
    v = Math.abs(num(v))
    if (v >= 1e6) return Math.round(v / 1e5) / 10 + 'M'  // 1.5M format (one decimal)
    if (v >= 1e3) {
      // For thousands, show one decimal if there are fractional thousands
      const thousands = v / 1e3
      if (thousands >= 100) {
        // 100k+ → no decimal (e.g., 150k not 150.0k)
        return Math.round(thousands) + 'k'
      } else {
        // Under 100k → one decimal (e.g., 33.6k not 34k)
  ...
  ui.innerHTML = `
    <div id="hm-head" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:#151f33;border-bottom:1px solid #86531f;cursor:move;flex-shrink:0">
      <div><b>HAMMER v8.11</b> <span style="opacity:.85">MAKE IT SO</span></div>
